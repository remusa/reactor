defmodule ReactorWeb.Auth do
  ...
  # admin
  def authenticate_admin(conn = %{assigns: %{admin_user: true}}), do: conn
  ...
  def logged_in_user(conn = %{assigns: %{current_user: %{}}}, _), do: _

  # not logged in
  def logged_in_user(conn = %{assigns: %{current_user: %{}}}, _) do
    conn
    |>put_status(:unauthorized)
    |>halt()
    |>json(%{error: "unauthorized"})
